...
class SQLStatement(BaseSQLStatement[exp.Expression]):
    ...
    def _has_limit_by(self) -> bool:
        """
        Recognize a per-group ClickHouse `LIMIT [n,] m [OFFSET n] BY expr` clause.

        Depending on the offset form, sqlglot attaches the `BY` expressions to
        either the LIMIT or the OFFSET node.
        """
        return any(
